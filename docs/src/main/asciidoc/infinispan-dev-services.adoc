...
If you want run the Infinispan Server container with Cross Site Replication configuration, the required configuration properties
are the following:
...
<2> Mcast port need to be different between sites if you are running more than one site.
